...
6. **Create early manifest** (fresh start only): Write a minimal manifest at `{WORKFLOW_DIR}/{feature}/aidlc-manifest.yaml` with `state.sharedPhases: []` and `state.status: "active"`. This enables the status line to display progress immediately. The full manifest fields are populated in Step 7 after assessment completes.
...
Before executing any action, create a task list showing all actions in this phase.

**Claude Code**: Use `TaskCreate` for each action. Mark `in_progress` when starting an action, `completed` when done.
**Other platforms**: Skip this step.

⚠️ Do NOT proceed to the first action until the progress tracker is created.
...
Update `{WORKFLOW_DIR}/{feature}/aidlc-manifest.yaml` (created during initialization) with full fields:
