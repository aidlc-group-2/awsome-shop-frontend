...
3. **Claude Code only — Status line setup**: If platform is Claude Code:
   - Copy `{PLATFORM_DIR}/skills/aidlc/scripts/aidlc-statusline.sh` to `{PLATFORM_DIR}/aidlc-statusline.sh` (if not already there)
   - Make it executable (`chmod +x`)
   - Create or update `.claude/settings.local.json` to include:
     ```json
...
4. Scan for manifests at `{WORKFLOW_DIR}/*/aidlc-manifest.yaml`
